    ...
    public class Tama {
        ...
        public void UpdateStats() {
            ...
            if (dateNow > (Time + new TimeSpan(0, 0, 6))) {
                ...
                //Health = (int)Math.Floor((dateNow - Time).TotalMilliseconds / 1000);
                Time += dateNow.Subtract(Time);
                //Health -= (int)(DateTime.Now - Time).TotalSeconds;
            }
            //Health -= (int)Math.Floor(DateTime.Now.Subtract(Time).TotalSeconds);
            //Health -= (int)Math.Floor(Time.Subtract(DateTime.Now).TotalSeconds);
        }
    }
}
